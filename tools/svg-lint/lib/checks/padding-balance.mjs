...
// (bottom gap exceeding top gap by more than cfg.clearance.paddingImbalance px). Box-like shapes
...
export const paddingBalance = {
  ...
  run(doc) {
    ...
    for (const s of shapes) {
      ...
      if (bottomGap - topGap > cfg.clearance.paddingImbalance) {
        out.push(warning({
          check: ID, code: 'bottom-heavy', line: s.line ?? 1, column: s.column ?? 1,
          message: `Box has ${round(topGap)}px above its first line but ${round(bottomGap)}px below its last; the imbalance exceeds ${cfg.clearance.paddingImbalance}px`,
          repair: {
            actual: `${round(topGap)} / ${round(bottomGap)} (top / bottom)`,
            expected: `difference ≤ ${cfg.clearance.paddingImbalance}px`,
